...
We  argue that GALE is the preferred choice for
...
assume  the existence of some fitness function $f$ that   converts {\em decisions} $d$ to
output objectives $o$; i.e.
\[o = f(d)\]
Note that this paper uses the term ``model'' as a synonym for $f$.
...
and do not recommend GALE for these $C(j)=0$ models.
...
only evaluates and compares $2log_2(N)$ models. Hence,
we strongly
recommend  learners like GALE for these $C(j)>>0$ models.
...
In that work,
multi-objective optimization was applied  systems containing hierarchical  constraints, for which the analysis could access  100\% knowledge of the internal structure.
That structural knowledge was exploited via {\em push} and {\em pull} strategies that use decisions made in one part of a system to reduce the search space in other parts of the same system. 

In order to distinguish this ``other approach''
we call these kind of problems {\em white box} or {\em structural}
optimization. In our opinion, based
on a reading of the current literature, functional
optimization is more widely-applicable hence more widely-used than structural optimization.
...
and
 $\left\{
\exists o \in \text{objectives} \;\mid\; where ($\prec,\succ$) tests if $x_o$ is (worse,better) than $y_o$.
...
\item
Multi-objective {\em differential evolution}: members of the frontier compete (and are possibly replaced) by candidates generated via extrapolation among  any three other members of the frontier~\cite{storn97,5601760,abbass01};
\item
...
For full  details on the maths models, including their decisions
...
\begin{center}
\includegraphics[width=3in]{barcharts_runtime_galeAbsolute_v2.png}
\end{center}
...
\includegraphics[width=3.5in]{barcharts_numeval_v2.png}
...
are:
\bi
\item Small maths problems such as those offered in the appendix;
